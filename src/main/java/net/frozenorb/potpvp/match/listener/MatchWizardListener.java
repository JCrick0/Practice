...
public final class MatchWizardListener implements Listener {
    ...
    public void onSnowBallHit(ProjectileHitEvent event) {
        if (event.getEntity().hasMetadata("BIGBLACKSNOWBALL")) {
    ...
    public void onDamage(EntityDamageEvent event) {
        if (event.getCause() == EntityDamageEvent.DamageCause.FALL) {
            if (event.getEntity() instanceof Player) {
                ...
                if (match != null) {
                    if (match.getKitType().getId().equalsIgnoreCase("wizard")) {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        snowball.setMetadata("BIGBLACKSNOWBALL", new FixedMetadataValue(PotPvPSI.getInstance(), true));
