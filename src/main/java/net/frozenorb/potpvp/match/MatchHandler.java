...
public final class MatchHandler {
	...
	private final Map<UUID, Match> playingMatchCache = new ConcurrentHashMap<>();
	@Getter(AccessLevel.PACKAGE)
	private final Map<UUID, Match> spectatingMatchCache = new ConcurrentHashMap<>();
	...
	public MatchHandler() {
		...
		Bukkit.getPluginManager().registerEvents(new MatchBridgeListener(), PotPvPSI.getInstance());
		Bukkit.getPluginManager().registerEvents(new MatchSkywarsListener(), PotPvPSI.getInstance());
	...
	public Match startMatch(List<MatchTeam> teams, KitType kitType, boolean ranked, boolean allowRematches) {
		...
		for (MatchTeam team : teams) {
			for (UUID member : team.getAllMembers()) {
				...
				if (isPlayingOrSpectatingMatch(memberPlayer)) {
					throw new IllegalArgumentException(UniqueIDCache.name(member) + " is already in a match!");
				}
			}
		}
		...
		Optional<Arena> openArenaOpt = arenaHandler.allocateUnusedArena(schematic ->
				schematic.isEnabled() &&
						...
						(kitType.getId().equalsIgnoreCase("ARCHER") || !schematic.isArcherOnly())
		);
	...
	public static boolean canUseSchematic(KitType kitType, ArenaSchematic schematic) {
		...
		if (kitId.equalsIgnoreCase("PEARLFIGHT")) return schematic.isPearlFightOnly();
		if (kitId.equalsIgnoreCase("BRIDGES")) return schematic.isBridgesOnly();
		...
		if (schematic.isPearlFightOnly()) return kitId.equalsIgnoreCase("PEARLFIGHT");
		...
		if (schematic.isBridgesOnly()) return kitId.equalsIgnoreCase("BRIDGES");
